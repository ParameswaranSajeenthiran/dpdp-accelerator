 ...
 * data (see tests/consents/README.md on tolerating a persistent environment). The one deliberate
...
export const test = base.extend<Fixtures>({
  ...
  consentCleanupTracker: async ({ consentAdminConsentApi }, use) => {
    ...
    for (const id of purposeIds) {
      // eslint-disable-next-line no-await-in-loop -- sequential cleanup, not perf-sensitive
