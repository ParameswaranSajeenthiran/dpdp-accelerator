...
4. Optional, for `tests/05-account` only: an Identity Server administrator account
   (`TEST_IS_ADMIN_USERNAME`/`TEST_IS_ADMIN_PASSWORD`, the stock `admin` works). Those tests
   delete the account they sign in as, so they create a throwaway user through SCIM2 instead of
   using a shared persona — without these they skip themselves.
5. Node.js 18+.
...
| `npm run test:account` | `tests/05-account` |
...
| `05-account/` | Self-service account deletion end to end. Destructive and irreversible, so each test creates and signs in as its own throwaway user rather than any shared persona — needs `TEST_IS_ADMIN_USERNAME`/`TEST_IS_ADMIN_PASSWORD` and skips itself without them |
