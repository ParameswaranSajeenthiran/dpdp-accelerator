...
import { secondaryTenantScimSurface, superTenantScimSurface } from '../../utils/scimProvisioning'
...
test.describe('Test-account provisioning', () => {
  test("01.02.01 - Provisions the per-run tenant's four personas and assigns their roles", async () => {
    ...
    try {
      const personas = await provisionPersonas(
        ...
        token,
        {
          consentAdmin: 'tenant-consent-admin@dpdp.test',
          user: 'tenant-user@dpdp.test',
          user2: 'tenant-user-2@dpdp.test',
          dpo: 'tenant-dpo@dpdp.test',
        },
  ...
  test("01.02.02 - Provisions the super tenant's four personas and assigns their roles", async () => {
