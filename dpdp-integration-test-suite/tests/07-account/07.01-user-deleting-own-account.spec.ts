...
test.describe('Self-service account deletion (UI)', () => {
   ...
   * they still have.
   */
  test('07.01.01 - A user deletes their own account, or raises a request when approval is required', async () => {
    const { page } = session!
  ...
  test('07.01.02 - Cancelling leaves the account untouched', async () => {
    const { page } = session!
    const menu = new UserProfileMenuPage(page)
  ...
  test('07.01.03 - The self-delete scope does not authorize deleting anybody else', async () => {
    /*
