 ...
 * no-match empty state. See tests/03-purposes/03.01-admin-creating-purposes.spec.ts for the
...
test.describe('Admin searching the Purposes list (UI)', () => {
  ...
  test('03.03.04 - A search with no matches shows the empty-results message', async ({
    browser,
  }) => {
    const consentAdminPage = await loginAsConsentAdmin(browser)
    const listPage = new PurposeListPage(consentAdminPage)
