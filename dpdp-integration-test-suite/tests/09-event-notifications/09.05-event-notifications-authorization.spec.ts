 ...
 * anywhere in this environment holding a strict subset (e.g. read-only), so 09.05.03 documents
...
test.describe('Event Notification authorization and access control', () => {
  test('09.05.01 - A Consent Admin sees and can open Events, Topics, and Subscriptions', async ({
    browser,
    ...
    // targets the item specifically by its button role to disambiguate the two.
  ...
  test('09.05.02 - A Data Principal without Event Notification scopes cannot access event routes', async ({
    browser,
    userEventApi,
  }) => {
  ...
  test('09.05.03 - A token without write scopes cannot perform write operations', async ({
    consentAdminEventApi,
    userEventApi,
  }) => {
  ...
  tenantTest(
    '09.05.04 - Missing, expired, or wrong-tenant tokens cannot access Event Notification APIs',
