...
test.describe('Admin viewing Subscriptions', () => {
  test('09.03.01 - The subscription list renders configuration and accepts pagination', async ({
    browser,
  ...
  test('09.03.02 - Status and delivery-mode filters narrow the list and Clear restores it', async ({
    browser,
    consentAdminEventApi,
  }) => {
  ...
  test('09.03.03 - Searching by a partial subscription, topic, or callback value finds matching rows', async ({
    browser,
    consentAdminEventApi,
  }) => {
  ...
  test('09.03.04 - Subscription details show configuration, timestamps, and deliveries', async ({
    browser,
    consentAdminEventApi,
  }) => {
  ...
  test('09.03.05 - An unknown subscription id shows load failure without leaking data', async ({ browser }) => {
    const page = await loginAsConsentAdmin(browser)
    try {
      const detailsPage = new SubscriptionDetailsPage(page)
