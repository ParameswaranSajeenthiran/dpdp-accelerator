 ...
 *
 * 04.09.01/04.09.02 below trigger that shared logic via a mutation (no server config needed, run
 * always). 04.09.03 instead waits on the literal scheduled job with no mutation at all, so it only
...
test.describe('Consent expiry reconciliation (API)', () => {
  ...
  test('04.09.03 - The background ConsentExpiryJob reconciles a lapsed consent within one scheduler cycle, with an accurate history timestamp', async ({
    ...
    // No mutation is performed on this consent anywhere in this test - unlike 04.09.02, the only
