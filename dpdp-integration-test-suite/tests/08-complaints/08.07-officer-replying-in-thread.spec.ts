 ...
 * 08.04-data-principal-replying-in-thread.spec.ts, which has neither).
 *
 ...
 * transition graph. 08.07.03 asserts that curated menu, not the backend's full one; the
...
test.describe("Complaint Officer replying in a case thread (UI)", () => {
  test('08.07.01 - Sending a public reply appends it to the activity feed', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.07.02 - Sending a reply with a status change transitions the complaint and records the transition', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test("08.07.03 - Only OPEN's curated next statuses (In Progress, Waiting on Client) appear in the status menu", async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.07.04 - Switching to "Internal note" posts a note the Data Principal never sees', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.07.05 - Resolving requires confirmation, and cancelling leaves the complaint open and the draft intact', async ({
    browser,
    userComplaintApi,
    officerComplaintApi,
  }) => {
  ...
  test('08.07.06 - Confirming the resolve dialog resolves the complaint and locks the composer', async ({
    browser,
    userComplaintApi,
    officerComplaintApi,
  }) => {
  ...
  test('08.07.07 - Sending a reply with a status change and an attachment transitions the complaint and uploads the file', async ({
    browser,
    userComplaintApi,
  }) => {
