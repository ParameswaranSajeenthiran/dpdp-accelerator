...
test.describe('Data Principal creating complaints (UI)', () => {
  test('08.01.01 - Submitting a complaint with a category and description shows a success banner with a reference id', async ({
    browser,
  }) => {
  ...
  test('08.01.02 - Submitting without selecting a category shows a validation error and does not submit', async ({
    browser,
  }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const listPage = new ComplaintListPage(dataPrincipalPage)
  ...
  test('08.01.03 - Submitting without a description shows a validation error and does not submit', async ({
    browser,
  }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const listPage = new ComplaintListPage(dataPrincipalPage)
  ...
  test('08.01.04 - Attaching a file before submitting carries it through to the created complaint', async ({
    browser,
  }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const listPage = new ComplaintListPage(dataPrincipalPage)
  ...
  test('08.01.05 - "Upload files" is disabled while a file is staged, and removing it lets a different file be attached', async ({
    browser,
  }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const listPage = new ComplaintListPage(dataPrincipalPage)
  ...
  test('08.01.06 - Cancelling the dialog discards the draft without creating a complaint', async ({ browser }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const listPage = new ComplaintListPage(dataPrincipalPage)
    await listPage.goto()
    await listPage.openSubmitDialog()
