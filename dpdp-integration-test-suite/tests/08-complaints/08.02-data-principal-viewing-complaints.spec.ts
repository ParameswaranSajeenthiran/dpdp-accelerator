...
test.describe('Data Principal viewing complaints (UI)', () => {
  test('08.02.01 - The complaint list shows reference id, category, status, submitted and updated columns', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.02.02 - A freshly submitted complaint appears in the list with its category and Open status', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.02.03 - Opening a complaint from the list navigates to its detail page showing the same reference id', async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test("08.02.04 - A complaint's detail page shows its category, description, submitted date, and an empty attachments tab", async ({
    browser,
    userComplaintApi,
  }) => {
  ...
  test('08.02.05 - Navigating to an unknown complaint id shows the not-found state with a way back to the list', async ({
    browser,
  }) => {
    const dataPrincipalPage = await loginAsUser(browser)
    const detailPage = new ComplaintDetailPage(dataPrincipalPage)
