...
public final class AppBootstrap {
    ...
    private static String sanitizeSqlForH2(String sql) {
        ...
        return sql.replaceAll("(?i)ENGINE\\s*=\\s*InnoDB", "")
                  ...
                  .replaceAll("_utf8mb4'", "'")
                  // MySQL's inline named "UNIQUE KEY name (cols)" table constraint isn't valid H2
                  // syntax - H2 only accepts the unnamed "UNIQUE (cols)" form here.
                  .replaceAll("(?i)UNIQUE\\s+KEY\\s+`[^`]+`\\s*(\\([^)]*\\))", "UNIQUE $1")
                  // MySQL's inline secondary "KEY name (cols)" index has no H2 equivalent inside
                  // CREATE TABLE; drop it here rather than emitting a separate CREATE INDEX, since
                  // these are non-unique perf indexes and this path is local/dev-only (see the
                  // isMysql branch in initDatabase, which executes the file verbatim in production).
                  .replaceAll("(?i),\\s*(?<!PRIMARY )KEY\\s+`[^`]+`\\s*\\([^)]*\\)", "")
                  .trim();
    }
}