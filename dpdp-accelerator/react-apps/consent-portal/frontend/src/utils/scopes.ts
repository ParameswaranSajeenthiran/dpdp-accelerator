...
/**
 * True for a DPO - someone who holds COMPLAINTS_READ_ANY but neither CONSENTS_READ_ANY nor
 * COMPLAINTS_READ_SELF. internal_login (CONSENTS_READ_SELF) can't be used to spot this: it is
 * granted to every signed-in user regardless of role, DPO included, so on its own it says
 * nothing about whether someone is a genuine consent subject. A DPO-only profile has no consents
 * of their own and no complaints filed as a Data Principal, so anything gated on "am I a
 * consent/complaint subject" (e.g. the Dashboard) should treat this profile as having none.
 */
