 ...
 */
public final class DPDPTenantContext {

    private DPDPTenantContext() {
    }
     ...
     * @throws DPDPCommonRuntimeException if no tenant context is available
     */
    public static String getOrganizationId() {

        String tenantDomain = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantDomain();
        if (tenantDomain == null || tenantDomain.trim().isEmpty()) {
            throw new DPDPCommonRuntimeException("Tenant domain is not available in the Carbon context.", null);
        }
